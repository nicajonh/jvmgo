...
func createStackTraceElements(tObj *heap.Object,threa *rtdata.Thread) []*StackTraceElement{
	// 由于栈顶两帧正在执行fillInStackTrace(int)和fillInStackTrace()，所以这两帧也需要跳过
	skip := distanceToObject(tObj.Class())+2
	//获取路过的帧这后的完整的数据的Java虚拟机栈
...
func (self *StackTraceElement) String() string{
	return fmt.Sprintf("%s.%s(%s:%s)",self.className, self.methodName, self.fileName, self.lineNumber)
}
